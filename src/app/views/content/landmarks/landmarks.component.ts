...
export class LandmarksComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllLandmarks(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'landmarks').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Landmarks.png";
              console.log(b);
            }
            this.landmarks.push(b);
            this.landmarks = this.landmarks.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
