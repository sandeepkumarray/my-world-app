...
@Component({
  selector: 'app-edit-landmarks',
  templateUrl: './edit-landmarks.component.html',
  styleUrls: ['./edit-landmarks.component.css']
})
export class EditLandmarksComponent implements OnInit {
  id: any = "";
  landmarkDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Landmarks.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getLandmarks(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Landmarks').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "landmarks")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'landmarks').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
