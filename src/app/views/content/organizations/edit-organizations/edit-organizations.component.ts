...
export class EditOrganizationsComponent implements OnInit {
  id: any = "";
  organizationDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Organizations.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getOrganizations(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
          ...
          this.organizationDic = dictionary;
    ...
    this.myworldService.getContentTypes('Organizations').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "organizations")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'organizations').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
  ...
  onBlur($event: any, field_name: string) {
    ...
    model.content_type = "organizations";
  ...
  addImage(event: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      aFile.content_type = "organizations";
