...
export class EditLocationsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Locations.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getLocations(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Locations').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "locations")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'locations').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
