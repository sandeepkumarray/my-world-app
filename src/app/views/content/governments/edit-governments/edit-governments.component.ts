...
@Component({
  selector: 'app-edit-governments',
  templateUrl: './edit-governments.component.html',
  styleUrls: ['./edit-governments.component.css']
})
export class EditGovernmentsComponent implements OnInit {
  id: any = "";
  governmentDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Governments.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getGovernments(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Governments').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "governments")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'governments').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
  ...
  onBlur($event: any, field_name: string) {
    ...
    model.content_type = "governments";
  ...
  addImage(event: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      aFile.content_type = "governments";
