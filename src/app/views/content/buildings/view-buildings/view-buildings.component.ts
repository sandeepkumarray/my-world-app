...
@Component({
  selector: 'app-view-buildings',
  templateUrl: './view-buildings.component.html',
  styleUrls: ['./view-buildings.component.scss']
})
export class ViewBuildingsComponent implements OnInit {
  id: any = "";
  contentAvailable: boolean = false;
  buildingDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    ...
    this.contentService.getBuildings(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Buildings').subscribe(res => {
      this.headerBackgroundColor = res.sec_color;
    });

    this.myworldService.getContentBlobObject(this.id, 'buildings').subscribe(res => {
      if(res != null){
        res.map(c => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "buildings")!;
