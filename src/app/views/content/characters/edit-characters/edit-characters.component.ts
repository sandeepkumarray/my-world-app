...
export class EditCharactersComponent implements OnInit {
  id: any = "";
  characterDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getCharacters(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Characters').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "characters")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'characters').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
