...
export class ReligionsComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllReligions(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'religions').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Religions.png";
              console.log(b);
            }
            this.religions.push(b);
            this.religions = this.religions.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
