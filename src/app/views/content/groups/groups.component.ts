...
export class GroupsComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllGroups(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'groups').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Groups.png";
              console.log(b);
            }
            this.groups.push(b);
            this.groups = this.groups.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
