...
export class ContinentsComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllContinents(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'continents').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Continents.png";
              console.log(b);
            }
            this.continents.push(b);
            this.continents = this.continents.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
