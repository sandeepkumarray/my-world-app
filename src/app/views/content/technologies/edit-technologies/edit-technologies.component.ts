...
export class EditTechnologiesComponent implements OnInit {
  id: any = "";
  technologieDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Technologies.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getTechnologies(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
          ...
          this.technologieDic = dictionary;
    ...
    this.myworldService.getContentTypes('Technologies').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "technologies")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'technologies').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
  ...
  onBlur($event: any, field_name: string) {
    ...
    model.content_type = "technologies";
  ...
  addImage(event: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      aFile.content_type = "technologies";
