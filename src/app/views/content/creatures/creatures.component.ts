...
export class CreaturesComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllCreatures(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'creatures').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Creatures.png";
              console.log(b);
            }
            this.creatures.push(b);
            this.creatures = this.creatures.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
