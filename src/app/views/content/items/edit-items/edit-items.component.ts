...
export class EditItemsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Items.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getItems(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Items').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "items")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'items').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
