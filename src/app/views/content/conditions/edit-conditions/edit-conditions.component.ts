...
@Component({
  ...
  templateUrl: './edit-conditions.component.html',
  styleUrls: ['./edit-conditions.component.css']
})
export class EditConditionsComponent implements OnInit {
  id: any = "";
  conditionDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getConditions(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Conditions').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "conditions")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'conditions').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
