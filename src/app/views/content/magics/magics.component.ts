...
@Component({
  selector: 'app-magics',
  templateUrl: './magics.component.html',
  styleUrls: ['./magics.component.css']
})
export class MagicsComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllMagics(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'magics').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Magics.png";
              console.log(b);
            }
            this.magics.push(b);
            this.magics = this.magics.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
