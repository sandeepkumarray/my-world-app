...
@Component({
  selector: 'app-edit-scenes',
  templateUrl: './edit-scenes.component.html',
  styleUrls: ['./edit-scenes.component.css']
})
export class EditScenesComponent implements OnInit {
  id: any = "";
  sceneDic: { [key: string]: string } = {};
  ...
  ngOnInit(): void {
    this.slides[0] = {
      src: './assets/img/cards/Scenes.png',
    };
    this.slides[1] = {
    ...
    let accountId = (this.authService.getUser() as (Users)).id;
    this.contentService.getScenes(accountId, this.id).subscribe({
      next: response => {
        if (response != null) {
    ...
    this.myworldService.getContentTypes('Scenes').subscribe(res => {
    ...
    this.myworldService.getUsersContentTemplate(accountId).subscribe(res => {
      ...
      this.ContentTemplate = contentTemplateModel.contents.find(c => c.content_type == "scenes")!;
  ...
  loadContentBlobObjects() {
    ...
    this.myworldService.getContentBlobObject(this.id, 'scenes').subscribe(res => {
      let allowedTotalContentSize = this.authService.getValue(constants.AllowedTotalContentSize);
