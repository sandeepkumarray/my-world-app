...
export class FlorasComponent implements OnInit {
  ...
  index(): void {
    ...
    this.contentService.getAllFloras(accountId).subscribe({
      next: (res) => {
        res.map(b => {
          this.myworldService.getContentBlobObject(b.id, 'floras').subscribe(conObjectResponse => {
            if (conObjectResponse) {
              ...
              b.image_url = url;
               ...
               b.image_url = "assets/img/cards/Floras.png";
              console.log(b);
            }
            this.floras.push(b);
            this.floras = this.floras.sort((a, b) => a.id! - b.id!);
          });
        });
      }
    });
